...
from . import Embedding, CNN
import sys
sys.path.append("..") 
from util import seq_len_to_mask


class PCNN(BasicModule.BasicModule):
    def __init__(self, cfg):
        super(PCNN, self).__init__()

        self.use_pcnn = cfg.model.use_pcnn
        ...
        if self.use_pcnn:
            self.fc_pcnn = nn.Linear(3 * len(cfg.model.kernel_sizes) * cfg.model.out_channels,
                                     len(cfg.model.kernel_sizes) * cfg.model.out_channels)
